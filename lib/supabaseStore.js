...
export async function loadStudySubmissions() {
  if (!hasSupabaseConfig) return [];

  const { data, error } = await supabase
    .from("study_submissions")
    .select("timestamp,participant_id,condition_order,background,preference_ranking,scenarios,satisfaction,payload")
  ...
  return (data || []).map((row) => ({
    ...
    background: row.background || row.payload?.background || {},
    preference_ranking: row.preference_ranking || row.payload?.preference_ranking || [],
    scenarios: row.scenarios || row.payload?.scenarios || [],
    satisfaction: row.satisfaction || row.payload?.satisfaction || {},
    summary_scores: row.payload?.summary_scores || {}
  }));
}
