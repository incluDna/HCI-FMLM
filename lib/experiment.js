...
// แก้ใหม่ — baseline (App A) ขึ้นก่อนเสมอ
export function conditionForCounter(counter) {
...
// แก้ใหม่: baseline = App A เสมอ, prototype = App B เสมอ
export function interfaceLabel(interfaceType) {
  return interfaceType === "baseline" ? "A" : "B";
}
...
export function computeTotals(scenario, selection) {
  ...
  return {
    ...
    walk_m: (fm?.walk_m || 0) + (lm?.walk_m || 0)
  };
}
